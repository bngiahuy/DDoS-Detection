...
import { Select, SelectTrigger, SelectValue, SelectContent, SelectItem } from './ui/select'; // Giả định component Select/Dropdown
...
const mockDatasetPreview = {
    headers: ['Packet ID', 'Timestamp', 'Source IP', 'Destination IP', 'Protocol', 'Length', 'Label'],
    data: [
        { id: 1, ts: '16:00:01', src: '192.168.1.10', dst: '10.0.0.1', proto: 'TCP', len: 64, label: 'Normal' },
        { id: 2, ts: '16:00:02', src: '192.168.1.11', dst: '10.0.0.2', proto: 'UDP', len: 128, label: 'Normal' },
        { id: 3, ts: '16:00:03', src: '203.0.113.5', dst: '10.0.0.3', proto: 'ICMP', len: 32, label: 'Attack' },
        { id: 4, ts: 4, src: '192.168.1.12', dst: '10.0.0.4', proto: 'TCP', len: 80, label: 'Normal' },
        { id: 5, ts: 5, src: '203.0.113.6', dst: '10.0.0.5', proto: 'UDP', len: 150, label: 'Attack' },
    ],
...
export default function CreatePreprocessing() {
    ...
    const totalPages = Math.ceil(mockDatasetPreview.totalRecords / mockDatasetPreview.pageSize);
                ...
                <>
                        ...
                        <Table className="w-full text-left table-auto border-collapse">
                            <TableHeader className="sticky top-0 bg-slate-800/80 backdrop-blur-sm">
                                <TableRow className="border-b border-purple-500/50">
                                    {mockDatasetPreview.headers.map((header, index) => (
                                        <th key={index} className="px-4 py-3 text-xs font-bold uppercase tracking-wider text-amber-400">
                                            {header}
                                        </th>
                                    ))}
                                </TableRow>
                            ...
                            <TableBody>
                                {mockDatasetPreview.data.map((row, index) => (
                                    <TableRow 
                                        key={index}
                                        className={`border-b border-slate-700/50 transition duration-150 ${index % 2 === 0 ? 'bg-slate-800' : 'bg-slate-800/70'} hover:bg-slate-700/50`}
                                    >
                                        {Object.values(row).map((value, idx) => (
                                            <TableCell key={idx} className={`px-4 py-2 text-sm ${idx === mockDatasetPreview.headers.length - 1 && value === 'Attack' ? 'text-red-400 font-medium' : 'text-slate-300'}`}>
                                                {value}
                                            </TableCell>
                                        ))}
                                    </TableRow>
                    ...
                    <div className="flex justify-between items-center mt-4 pt-3 border-t border-slate-700">
                        <p className="text-sm text-slate-400">
                            Hiển thị **{mockDatasetPreview.data.length}** / **{sampleSize}** bản ghi mẫu. 
                            (Tổng: {mockDatasetPreview.totalRecords.toLocaleString()} records)
                        </p>
            ...
            <Card className="w-full max-w-7xl mx-auto bg-slate-900 border-t-4 border-purple-500 rounded-xl shadow-2xl text-slate-100 overflow-hidden">
                <CardHeader className="flex flex-col md:flex-row items-start md:items-center justify-between p-6 sm:p-8 border-b border-slate-700/50 space-y-4 md:space-y-0">
                    ...
                    <div className="flex items-center space-x-3 w-full md:w-auto">
                        ...
                        <label htmlFor="load-pipeline" className="text-slate-400 whitespace-nowrap">Load existing pipeline:</label>
                        <Select onValueChange={(value) => console.log('Loaded:', value)} >
                            <SelectTrigger id="load-pipeline" className="bg-slate-800 border-slate-700 text-slate-300 hover:border-purple-500/50">
                ...
                <CardContent className="p-6 sm:p-8">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <div className="lg:col-span-2 space-y-6">
                            {/* Trường Name */}
                            <div>
                                <label htmlFor="pipeline-name" className="block text-sm font-medium text-slate-300 mb-2">
                                    Pipeline Name
                                </label>
                                <Input
                                    id="pipeline-name"
                        ...
                        <div className="lg:col-span-1 space-y-6">
                            ...
                            <div>
                                <label htmlFor="dataset-version" className="block text-sm font-medium text-slate-300 mb-2">
                                    Dataset Version
                                </label>
                                <Select onValueChange={(value: string) => setSelectedDataset(value)}>
                                    <SelectTrigger id="dataset-version" className="bg-slate-800 border-slate-700 text-slate-300 hover:border-amber-500/50">
                            ...
                            <div>
                                ...
                                <Input
                                    ...
                                    className="w-full bg-slate-800 border-slate-700 text-slate-100 placeholder:text-slate-500 focus:border-purple-500"
                                />
                                <p className="text-xs text-slate-500 mt-1">Max number of records to load/process.</p>
                            </div>
                            ...
                            <div className="pt-4 border-t border-slate-700">
                                ...
                                <div className="p-4 mt-3 border border-purple-600/50 rounded-lg bg-slate-800/80 text-slate-400 italic">
                                    // Nội dung cho phần Preprocessing Method sẽ được thêm vào đây...
                                </div>
                            </div>
